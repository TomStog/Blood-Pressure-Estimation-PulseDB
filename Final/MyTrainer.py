...
def CPT(y_true, y_pred, threshold):
    abs_errors = np.abs(y_true - y_pred)
    within = np.mean(abs_errors <= threshold) * 100  # percentage
    return within
...
class Model_Trainer:
    def __init__(self, model, criterion_BP, optimizer_BP, device, settings_yml, batch_size=32, num_epochs=10, save_states=False, save_final=False):
        ...
        for Epoch in range(Start_Epoch, Start_Epoch+self.Num_Epoch):
            try:
                ...
                with PB.ProgressBar(widgets=widgets, max_value=len(Train)) as bar:
                    ...
                    Epoch_Train_CPT5 = CPT(Epoch_BP_Labels, Epoch_BP_Preds, 5)
                    Epoch_Train_CPT10 = CPT(Epoch_BP_Labels, Epoch_BP_Preds, 10)
                    Epoch_Train_CPT15 = CPT(Epoch_BP_Labels, Epoch_BP_Preds, 15)
                ...
                Writer_CPT5_Dict = {'Train': Epoch_Train_CPT5}
                Writer_CPT10_Dict = {'Train': Epoch_Train_CPT10}
                Writer_CPT15_Dict = {'Train': Epoch_Train_CPT15}
                ...
                for name, Test in zip(Test_Names, Test_List):
                    ...
                    Epoch_Test_Loss = self.BP_Loss_Fun(torch.from_numpy(Epoch_Preds),
                    ...
                    Epoch_Test_CPT5 = CPT(Epoch_Labels, Epoch_Preds, 5)
                    Epoch_Test_CPT10 = CPT(Epoch_Labels, Epoch_Preds, 10)
                    Epoch_Test_CPT15 = CPT(Epoch_Labels, Epoch_Preds, 15)
                    # Print a summary
                    print(
                    ...
                    Writer_CPT5_Dict.update({Test_Name: Epoch_Test_CPT5})
                    Writer_CPT10_Dict.update({Test_Name: Epoch_Test_CPT10})
                    Writer_CPT15_Dict.update({Test_Name: Epoch_Test_CPT15})
                ...
                Writer.add_scalars('CPT5', Writer_CPT5_Dict, Epoch)
                Writer.add_scalars('CPT10', Writer_CPT10_Dict, Epoch)
                Writer.add_scalars('CPT15', Writer_CPT15_Dict, Epoch)
                ################################################################################################
